...
6. MCP gateways are a must, the agent needs to find the right tool from potentially hundreds/thousands of tools at a large enough company that we need tohave methods of injecting only the needed tools for a workflow task to be completed.
